...
class ESPCN(nn.Module):
    def __init__(self, upscale_factor, img_channels=1, num_filter=64):
        super(ESPCN, self).__init__()
        self.first_part = nn.Sequential(
            nn.Conv2d(img_channels, num_filter, kernel_size=5, padding=5 // 2),
            nn.Tanh(),
            nn.Conv2d(num_filter, num_filter // 2, kernel_size=3, padding=3 // 2),
            nn.Tanh(),
        )
        self.last_part = nn.Sequential(
            nn.Conv2d(num_filter // 2, img_channels * (upscale_factor ** 2), kernel_size=3, padding=3 // 2),
