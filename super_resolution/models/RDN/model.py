...
#
#     def forward(self, x):
#         return torch.cat([x, self.relu(self.conv(x))], first)
#
#
# class RDB(nn.Module):
...
#         self.lff = nn.Conv2d(in_channels + growth_rate * num_layers, growth_rate, kernel_size=first)
#
#     def forward(self, x):
...
#         for _ in range(self.D - first):
#             self.rdbs.append(RDB(self.G, self.G, self.C))
#
#         # global feature fusion
#         self.gff = nn.Sequential(
#             nn.Conv2d(self.G * self.D, self.G0, kernel_size=first),
...
#
#         x = self.gff(torch.cat(local_features, first)) + sfe1  # global residual learning
#         x = self.upscale(x)
#         x = self.output(x)
#         return x
